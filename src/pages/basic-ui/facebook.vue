<template>
<div>
    <h3>Click an button to link a facebook page  </h3><br>
<div>
  <h4 v-if="name">Logged in as {{name}}</h4>
  <h4 v-if="pageName1" >Your page name is {{pageName1}}</h4>
      <img  class="rounded-circle mt-5" :src="image" width="20%"><span class="font-weight-bold"></span><span class="text-black-50"></span></div>
      <br>
 <button @click="loginWithFb()"  class="fb connect">Sign in with Facebook</button><br><br>
<button @click="getToken()" class="button button2">Token</button>
<button @click="info()" class="button button2">Your Info</button>
<button @click="pages()" class="button button2">Your pages</button>
</div>
</template>


<script>
import axios from 'axios'
import jwtDecode from 'jwt-decode'
import { async } from 'q';
export default {
  name: 'facebook',
  
  data(){
      return{
          id : "",
          pageName1:"",
          email : "",
          pageId : "",
          pageToken : "",
          name : "",
          profile : null,
          token : "",
          image : ""
      }
  },
    methods:{
        loginWithFb() {
            const user = jwtDecode(localStorage.getItem('accessToken'));
            window.location = `http://localhost:2400/auth/facebook?id=${user._id}`
            ;
        },
        async getToken(){
        const res = await axios.get('http://localhost:2400/token/62977efa51e0729b067c3c9f')
        this.token = res.data.user.facebookAccessToken;
        console.log(token);
        },
       async info(){
            const response = await axios.get(`https://graph.facebook.com/v14.0/me?fields=picture%2Cname&access_token=${this.token}`)
            console.log(response)
            this.name = response.data.name;
            this.image = response.data.picture.data.url;
            console.log(image);
        },
      async pages(){
          const res2 = await axios.get(`https://graph.facebook.com/v14.0/me/accounts?access_token=${this.token}`)
          this.pageName1 = res2.data.data[0].name;
          this.pageToken = res2.data.data[0].access_token;
          this.pageId = res2.data.data[0].id;
        }
    }
}
</script>
<style scoped >
@import url('https://fonts.googleapis.com/css2?family=Hind+Siliguri:wght@300&family=Montserrat:wght@100&display=swap');

.button2 {
    background-color: #008CBA;
      border: none;
  color: white;
  padding: 15px 32px;
  text-align: center;
  text-decoration: none;
  display: inline-block;
  font-size: 16px;
  margin: 4px 2px;
  cursor: pointer;
    } /* Blue */

button.fb {
    font-family: Lucida Grande, Helvetica Neue, Helvetica, Arial, sans-serif;
    display: inline-block;
    font-size: 14px;
    padding: 13px 30px 15px 44px;
    background: #3A5A97;
    color: #fff;
    text-shadow: 0 -1px 0 rgba(0,0,20,.4);
    text-decoration: none;
    line-height: 1;
    position: relative;
    border-radius: 5px;
    margin-left: 350px;
}

.connect:before {
    display: inline-block;
    position: relative;
    background-image: url(data:image/png;base64,);
    height: 23px;
    background-repeat: no-repeat;
    background-position: 0px 3px;
    text-indent: -9999px;
    text-align: center;
    width: 29px;
    line-height: 23px;
    margin: -8px 7px -7px -30px;
    padding: 2 25px 0 0;
    content: "f";
}
h4{
  margin-left: 320px
}
img{
    margin-left: 350px;
    margin-top: 40px;
}
h3{
    font-family: 'Hind Siliguri', sans-serif;
    text-align: center;
}
@import url('https://fonts.googleapis.com/css?family=Lato:100&display=swap');

body, html {
  width: 100%;
  height: 100%;
  margin: 0;
  padding: 0;
  background: #5CA4EA;
  overflow: hidden;
  font-family: 'Lato', sans-serif;
}

.container {
  width: 400px;
  height: 400px;
  position: absolute;
  left: 50%;
  top: 50%;
  transform: translate(-50%, -50%);
  display: flex;
  justify-content: center;
  align-items: center;
}

.center {
  width: 180px;
  height: 60px;
  position: absolute;
}

.btn {
  width: 180px;
  height: 60px;
  cursor: pointer;
  background: transparent;
  border: 1px solid #91C9FF;
  outline: none;
  transition: 1s ease-in-out;
}

svg {
  position: absolute;
  left: 15px;
  top: 0;
  fill: none;
  stroke: #fff;
  stroke-dasharray: 150 480;
  stroke-dashoffset: 150;
  transition: 1s ease-in-out;
}

.btn:hover {
  transition: 1s ease-in-out;
  background: #4F95DA;
}

.btn:hover svg {
  stroke-dashoffset: -480;
}

.btn span {
  color: white;
  font-size: 18px;
  font-weight: 100;
}  
 

</style>